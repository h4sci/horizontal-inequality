...
//Location, with a balanced set of assets between countries
...
tab hv025   //DHS variable for Region
tab hv025, nol
drop if hv025>2

gen location =.
label variable location "geographical location"
replace location = hv025

*Generate group id's
egen g_id = group(location) 
sort g_id
...
collapse w_grp_ai_mean hv007 gini_wlth g_shr, by(g_id)
gen n_grp = _N
...
generate str category = "location"
gen str match = "btwn"
gen str note = "."


export excel country_code measure category hv007 ggini_abs ggini_rel gini_wlth overall_mu match note in 1 using "\\gess-fs.d.ethz.ch\home$\tsamuel\Desktop\PR_GHANA\results.xls", sheet("sam") firstrow(variables) replace 
import excel "\\gess-fs.d.ethz.ch\home$\tsamuel\Desktop\PR_GHANA\results.xls", sheet("sam") firstrow clear
_getfilename `x'

local z = r(filename)
save location_ai_btwn_wgt_`z', replace
...
use ddata_ai_1, clear
       foreach num of numlist 2/25 {
               append using ddata_ai_`num'
       }
